...
#ifdef EVDO_DT_SUPPORT
{
    ...
    if (le16_to_cpu(dev->descriptor.idVendor) == VIATELECOM_VENDOR_ID && le16_to_cpu(dev->descriptor.idProduct) == VIATELECOM_PRODUCT_ID) {
        val = portdata->dtr_state ? 1:0;
        return usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), 0x01, 0x40, val, ifNum, NULL, 0, USB_CTRL_SET_TIMEOUT);
    } else {
        if (portdata->dtr_state)
            val |= 0x01;
        if (portdata->rts_state)
            val |= 0x02;
        return usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0), 0x22, 0x21, val, ifNum, NULL, 0, USB_CTRL_SET_TIMEOUT);
    }
}
#else
	if (portdata->dtr_state)
